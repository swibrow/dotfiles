...
- Secrets are age-encrypted inline in `dot_config/mise/config.toml` via `mise run secret:set NAME` / `secret:rm NAME` (key at `~/.config/mise/age.txt`, not chezmoi-managed). The macOS Keychain (service=`env`, account=VAR_NAME) + `keychain-secret` helper is still available for ad hoc use outside mise, but is no longer the default for mise-consumed secrets.
